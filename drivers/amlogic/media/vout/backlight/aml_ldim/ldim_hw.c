 ...
 */
	...
	if (rw_sel == LDIM_VPU_DMA_WR) { //write mif
		...
		ldim_wr_vcbus(VPU_DMA_RDMIF0_BADR0,
			      ldim_drv->rmem->rd_mem_paddr1);
		ldim_wr_vcbus(VPU_DMA_RDMIF0_BADR1,
			      ldim_drv->rmem->rd_mem_paddr1);
		ldim_wr_vcbus(VPU_DMA_RDMIF0_BADR2,
			      ldim_drv->rmem->rd_mem_paddr1);
		ldim_wr_vcbus(VPU_DMA_RDMIF0_BADR3,
			      ldim_drv->rmem->rd_mem_paddr1);
		/* stride: transfer number(128 bits) */
		ldim_wr_vcbus_bits(VPU_DMA_RDMIF0_CTRL,
				   (zone_num + 7) / 8, 0, 13);
		//reset index to 0
		ldim_wr_vcbus_bits(VPU_DMA_RDMIF0_CTRL, 1, 30, 2);
		//vsync source none for init
		ldim_wr_vcbus_bits(VPU_DMA_RDMIF0_CTRL, 0, 16, 8);
	}
}

static void ldim_hw_vpu_dma_mif_en(int rw_sel, int flag)
{
	unsigned int en;
...
static void ldim_hw_vpu_dma_mif_en_tm2b(int rw_sel, int flag)
{
	unsigned int en;

	en = flag ? 0x2 : 0x0; //encl_vsync
	if (rw_sel == LDIM_VPU_DMA_WR)
		ldim_wr_vcbus_bits(VPU_DMA_WRMIF_CTRL, en, 13, 1);
	else
		ldim_wr_vcbus_bits(VPU_DMA_RDMIF0_CTRL, en, 16, 8);
}
...
void ldim_hw_remap_en(int flag)
{
	...
	if (flag) {
		//ldim_wr_reg(REG_LD_MISC_CTRL0, 0x706);
		ldim_wr_reg_bits(REG_LD_MISC_CTRL0, 1, 8, 1);
		ldim_wr_reg_bits(REG_LD_MISC_CTRL0, 1, 2, 1);
	} else {
		//ldim_wr_reg(REG_LD_MISC_CTRL0, 0x600);
		ldim_wr_reg_bits(REG_LD_MISC_CTRL0, 0, 2, 1);
		ldim_wr_reg_bits(REG_LD_MISC_CTRL0, 0, 8, 1);
	}
...
static void ldim_hw_remap_disable_tm2(void)
{
	...
	misc_ctrl = ldim_rd_reg(REG_LD_MISC_CTRL0);
	misc_ctrl &= ~((1 << 2) | (1 << 8));
...
static void ldim_hw_remap_ram_update(struct LDReg_s *nprm)
{
	struct aml_ldim_driver_s *ldim_drv = aml_ldim_get_driver();
	unsigned int misc_ctrl;

	if (ldim_drv->remap_ram_flag == 0)
		return;
	if (ldim_drv->remap_mif_flag > 0) {
		LDIMERR("%s: exit for remap_mif_flag %d\n",
			__func__, ldim_drv->remap_mif_flag);
		return;
	}
			      ...
			      unsigned int hvcnt_bypass)
{
	...
	if (ldim_drv->remap_ram_flag) {
		if (ldim_drv->remap_ram_step == 0) {
			/* disable remap and rdmif for step1 */
			ldim_hw_remap_disable_tm2();
			ldim_drv->remap_mif_flag = 0;
			ldim_drv->remap_ram_step = 1;
			return;
		}
	}
...
void ldim_remap_reg_dump_tm2(void)
{
	struct aml_bl_drv_s *bl_drv = aml_bl_get_driver();
	struct aml_ldim_driver_s *ldim_drv = aml_ldim_get_driver();
	unsigned int data, val, reg, misc_ctrl;
	unsigned char avg_update_en_bak, matrix_update_en_bak, remap_en;
	int temp;
	char *buf;
	int i, j, k, len;
	...
	avg_update_en_bak = ldim_drv->avg_update_en;
	matrix_update_en_bak = ldim_drv->matrix_update_en;
	remap_en = ldim_drv->remap_en ? 1 : 0;
	ldim_drv->avg_update_en = 0;
	ldim_drv->matrix_update_en = 0;
	ldim_hw_remap_disable_tm2();
	ldim_drv->remap_mif_flag = 0;
	...
	data = ldim_rd_reg(REG_LD_RGB_MOD);
	...
	data = ldim_rd_reg(REG_LD_BLK_HVNUM);
	...
	len = 0;
	reg = REG_LD_REFLECT_DGR_BASE_TM2;
	for (i = 0; i < 5; i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++)
	...
	len = 0;
	reg = REG_LD_REFLECT_DGR_BASE_TM2 + 5;
	for (i = 0; i < 5; i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++)
			len += sprintf(buf + len, " %d,",
	...
	len = 0;
	reg = REG_LD_REFLECT_DGR_BASE_TM2 + 10;
	data = ldim_rd_reg(reg);
	for (j = 0; j < 4; j++)
		len += sprintf(buf + len, " %d,",  (data >> (8 * j)) & 0x3f);
	pr_info("reg_LD_Reflect_Xdgr[4]=%s\n", buf);

	data = ldim_rd_reg(REG_LD_HVGAIN);
	...
	len = 0;
	data = ldim_rd_reg(REG_LD_BKLIT_VLD);
	for (j = 0; j < 32; j++)
		len += sprintf(buf + len, " %d,", (data >> j) & 0x1);
	pr_info("reg_LD_BkLit_valid[32]=%s\n", buf);

	len = 0;
	reg = REG_LD_BLK_HIDX_BASE_TM2;
	for (i = 0; i < ((LD_BLK_LEN_H + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_BLK_LEN_H)
				break;
			val = (data >> (16 * j)) & 0x3fff;
			if (val & (1 << 13))
				temp = val - 0x4000;
			else
				temp = val;
			len += sprintf(buf + len, " %d,", temp);
		}
	}
	pr_info("reg_LD_BLK_Hidx[%d]=%s\n", LD_BLK_LEN_H, buf);

	len = 0;
	reg = REG_LD_BLK_VIDX_BASE_TM2;
	for (i = 0; i < ((LD_BLK_LEN_V + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_BLK_LEN_V)
				break;
			val = (data >> (16 * j)) & 0x3fff;
			if (val & (1 << 13))
				temp = val - 0x4000;
	...
	len = 0;
	reg = REG_LD_LUT_HDG_BASE_TM2;
	for (i = 0; i < ((LD_LUT_LEN + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_LUT_LEN)
				break;
	...
	len = 0;
	reg = REG_LD_LUT_HDG_BASE_TM2 + 0x10;
	for (i = 0; i < ((LD_LUT_LEN + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_LUT_LEN)
	...
	len = 0;
	reg = REG_LD_LUT_VDG_BASE_TM2 + 0x10;
	for (i = 0; i < ((LD_LUT_LEN + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_LUT_LEN)
	...
	len = 0;
	reg = REG_LD_LUT_VHK_BASE_TM2 + 0x10;
	for (i = 0; i < ((LD_LUT_LEN + 1) / 2); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= LD_LUT_LEN)
	...
	len = 0;
	reg = REG_LD_LUT_VHK_NEGPOS_BASE_TM2;
	for (i = 0; i < ((LD_LUT_LEN + 3) / 4); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++) {
			if ((i * 4 + j) >= LD_LUT_LEN)
				break;
	...
	len = 0;
	reg = REG_LD_LUT_VHK_NEGPOS_BASE_TM2 + 8;
	for (i = 0; i < ((LD_LUT_LEN + 3) / 4); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++) {
			if ((i * 4 + j) >= LD_LUT_LEN)
	...
	len = 0;
	reg = REG_LD_LUT_VHO_NEGPOS_BASE_TM2;
	for (i = 0; i < ((LD_LUT_LEN + 3) / 4); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++) {
			if ((i * 4 + j) >= LD_LUT_LEN)
	...
	len = 0;
	reg = REG_LD_LUT_VHO_NEGPOS_BASE_TM2 + 8;
	for (i = 0; i < ((LD_LUT_LEN + 3) / 4); i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 4; j++) {
			if ((i * 4 + j) >= LD_LUT_LEN)
	...
	data = ldim_rd_reg(REG_LD_FRM_HBLAN_VHOLS);
	pr_info("reg_LD_LUT_VHo_LS=%d\n", (data >> 16) & 0x7);

	data = ldim_rd_reg(REG_LD_LUT_LEXT_BASE_TM2);
	pr_info("reg_LD_LUT_Hdg_LEXT_0=%d\n", data & 0x3ff);
	pr_info("reg_LD_LUT_Vdg_LEXT_0=%d\n", (data >> 20) & 0x3ff);
	pr_info("reg_LD_LUT_VHk_LEXT_0=%d\n", (data >> 10) & 0x3ff);

	data = ldim_rd_reg(REG_LD_LUT_LEXT_BASE_TM2 + 1);
	...
	len = 0;
	pr_info("REG_LD_RGB_LUT_BASE=\n");
	misc_ctrl = ldim_rd_reg(REG_LD_MISC_CTRL0);
	misc_ctrl &= ~(3 << 9);
	misc_ctrl |= (1 << 8);
	ldim_wr_reg(REG_LD_MISC_CTRL0, misc_ctrl);
	k = 0;
	reg = REG_LD_RGB_LUT_BASE;
	for (i = 0; i < 768; i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= 1536)
				break;
			val = (data >> (16 * j)) & 0xffff;
			len += sprintf(buf + len, " %d,", val);
			k++;
		}
		if (k >= 32) {
	...
	len = 0;
	pr_info("LD_MATRIX_LUT_RAM=\n");
	k = 0;
	reg = REG_LD_MATRIX_BASE;
	for (i = 0; i < 768; i++) {
		data = ldim_rd_reg(reg + i);
		for (j = 0; j < 2; j++) {
			if ((i * 2 + j) >= 1536)
					 ...
					 unsigned int hist_hnum)
{
	...
	if (resolution == 0) {
		...
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((vend9 & 0x1fff) << 16) | (vend8 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend11 & 0x1fff) << 16) | (hend10 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((vend11 & 0x1fff) << 16) | (vend10 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend13 & 0x1fff) << 16) | (hend12 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((vend13 & 0x1fff) << 16) | (vend12 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend15 & 0x1fff) << 16) | (hend14 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((vend15 & 0x1fff) << 16) | (vend14 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend17 & 0x1fff) << 16) | (hend16 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend19 & 0x1fff) << 16) | (hend18 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend21 & 0x1fff) << 16) | (hend20 & 0x1fff)));
		ldim_wr_vcbus(LDIM_STTS_HIST_SET_REGION,
			      (((hend23 & 0x1fff) << 16) | (hend22 & 0x1fff)));
		/*h region number*/
			      ...
			      unsigned int hist_vnum, unsigned int hist_hnum)
{
	...
	ldim_vpu_dma_mif_set(LDIM_VPU_DMA_RD, hist_vnum, hist_hnum);
}
			       ...
			       unsigned int hist_vnum, unsigned int hist_hnum)
{
	...
	//config vpu dma
	ldim_vpu_dma_mif_set_tm2b(LDIM_VPU_DMA_WR, hist_vnum, hist_hnum);
	ldim_vpu_dma_mif_set_tm2b(LDIM_VPU_DMA_RD, hist_vnum, hist_hnum);
}
