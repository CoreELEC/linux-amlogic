/*
 * drivers/amlogic/media/vout/peripheral_lcd/peripheral_lcd_dev.c
 *
...
static struct spi_board_info peripheral_lcd_spi_info = {
...
static int peripheral_lcd_dev_probe_flag;
...
static int peripheral_lcd_dev_init_table_dynamic_size_load(
...
static int peripheral_lcd_dev_init_table_fixed_size_load(
		struct device_node *of_node,
		struct per_lcd_dev_config_s *per_lcd_dev_conf, int flag)
{
...
static int peripheral_lcd_dev_tablet_init_dft_malloc(void)
{
...
static int peripheral_lcd_dev_table_init_save(struct per_lcd_dev_config_s
					      *per_lcd_dev_conf)
{
...
int peripheral_lcd_dev_get_config_from_dts(struct device_node *np, int index)
{
	struct device_node *child;
	char per_lcd_propname[20];
	unsigned int para[20], val;
	const char *str;
	int ret, i;
	...
	ret = of_property_read_u32_array(child, "resolution",
					 para, 2);
	if (ret) {
	...
	ret = of_property_read_u32_array(child, "per_lcd_attr",
					 para, 6);
	if (ret) {
	...
	ret = peripheral_lcd_dev_tablet_init_dft_malloc();
	if (ret)
		return -1;
	switch (per_lcd_dev_conf.type) {
	case PER_DEV_TYPE_SPI:
		/* get spi config */
		peripheral_lcd_drv->spi_info = &peripheral_lcd_spi_info;

		ret = of_property_read_u32(child, "spi_bus_num", &val);
		if (ret) {
			LCDERR("failed to get spi_bus_num\n");
		} else {
			peripheral_lcd_spi_info.bus_num = val;
			if (peripheral_lcd_debug_print) {
				LCDPR("spi bus_num: %d\n",
				      peripheral_lcd_spi_info.bus_num);
			}
		}
		...
		if (ret) {
			...
			peripheral_lcd_spi_info.chip_select = val;
			if (peripheral_lcd_debug_print) {
				LCDPR("spi chip_select: %d\n",
				      peripheral_lcd_spi_info.chip_select);
			}
		}
		...
		if (ret) {
			...
			peripheral_lcd_spi_info.max_speed_hz = val;
			if (peripheral_lcd_debug_print) {
				LCDPR("spi max_speed_hz: %d\n",
				      peripheral_lcd_spi_info.max_speed_hz);
			}
		}
		...
		if (ret) {
			...
			peripheral_lcd_spi_info.mode = val;
			if (peripheral_lcd_debug_print)
				LCDPR("spi mode: %d\n",
				      peripheral_lcd_spi_info.mode);
		}
	...
	case PER_DEV_TYPE_MCU_8080:
		...
		if (ret) {
			...
			for (i = 0; i < per_lcd_dev_conf.max_gpio_num; i++)
				peripheral_lcd_gpio_probe(i);
			if (peripheral_lcd_debug_print)
				LCDPR("max_gpio_num: %d\n",
				      per_lcd_dev_conf.max_gpio_num);
		}
		ret = of_property_read_u32_array(child, "mcu_attr",
						 para, 13);
		if (ret) {
	...
	if (ret) {
		...
		goto peripheral_lcd_get_config_init_table_err;
	} else {
	...
	if (per_lcd_dev_conf.cmd_size == LCD_EXT_CMD_SIZE_DYNAMIC) {
		ret = peripheral_lcd_dev_init_table_dynamic_size_load(
			child, &per_lcd_dev_conf, 1);
		if (ret)
			goto peripheral_lcd_get_config_init_table_err;
		peripheral_lcd_dev_init_table_dynamic_size_load(
			child, &per_lcd_dev_conf, 0);
	} else {
		ret = peripheral_lcd_dev_init_table_fixed_size_load(
			child, &per_lcd_dev_conf, 1);
		if (ret)
			goto peripheral_lcd_get_config_init_table_err;
		peripheral_lcd_dev_init_table_fixed_size_load(
			child, &per_lcd_dev_conf, 0);
	}
	if (ret == 0)
	...
	if (per_lcd_dev_conf.init_loaded > 0) {
		ret = peripheral_lcd_dev_table_init_save(&per_lcd_dev_conf);
		if (ret)
			goto peripheral_lcd_get_config_init_table_err;
	}
	...
	return 0;
peripheral_lcd_get_config_init_table_err:
	kfree(table_init_on_dft);
	kfree(table_init_off_dft);
	return -1;
}

static int peripheral_lcd_dev_add_driver(struct peripheral_lcd_driver_s
					 *peripheral_lcd_drv)
{
	int index = peripheral_lcd_drv->dev_index;
	int ret = 0;

	switch (per_lcd_dev_conf.type) {
	case PER_DEV_TYPE_SPI:
		ret = peripheral_lcd_spi_driver_add(peripheral_lcd_drv);
		break;
	case PER_DEV_TYPE_MCU_8080:
		break;
	case PER_DEV_TYPE_NORMAL:
	...
	if (strcmp(per_lcd_dev_conf.name, "intel_8080") == 0)
		ret = peripheral_lcd_dev_intel_8080_probe(peripheral_lcd_drv);
	else if (strcmp(per_lcd_dev_conf.name, "spi") == 0)
		ret = peripheral_lcd_dev_spi_probe(peripheral_lcd_drv);
	else
	...
	if (ret) {
		...
		peripheral_lcd_dev_probe_flag = 1;
...
static int peripheral_lcd_dev_remove_driver(struct peripheral_lcd_driver_s
					    *peripheral_lcd_drv)
{
	int index = peripheral_lcd_drv->dev_index;
	int ret = -1;
	...
	if (strcmp(per_lcd_dev_conf.name, "intel_8080") == 0)
		ret = peripheral_lcd_dev_intel_8080_remove(peripheral_lcd_drv);
	else if (strcmp(per_lcd_dev_conf.name, "spi") == 0)
		ret = peripheral_lcd_dev_spi_remove(peripheral_lcd_drv);
	else
		LCDPR("%s: unsopport device driver: %s(%d)\n",
		      __func__, per_lcd_dev_conf.name, index);
	...
	if (ret)
		...
		peripheral_lcd_dev_probe_flag = 0;
	...
	switch (per_lcd_dev_conf.type) {
	case PER_DEV_TYPE_SPI:
		ret = peripheral_lcd_spi_driver_remove(peripheral_lcd_drv);
		break;
	case PER_DEV_TYPE_MCU_8080:
		break;
	case PER_DEV_TYPE_NORMAL:
...
int peripheral_lcd_dev_probe(struct platform_device *pdev)
{
	...
	peripheral_lcd_drv->per_lcd_dev_conf = &per_lcd_dev_conf;
	peripheral_lcd_dev_get_config_from_dts(pdev->dev.of_node,
					       peripheral_lcd_drv->dev_index);
	peripheral_lcd_dev_add_driver(peripheral_lcd_drv);
	LCDPR("%s OK\n", __func__);
	return 0;
}
...
int peripheral_lcd_dev_remove(struct platform_device *pdev)
{
	...
	peripheral_lcd_dev_remove_driver(peripheral_lcd_drv);
	LCDPR("%s OK\n", __func__);
	return 0;
}
