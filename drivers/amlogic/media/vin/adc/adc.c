...
int adc_set_filter_ctrl(bool on, enum filter_sel module_sel, void *data)
{
	...
	case FILTER_ATV_DEMOD:
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			...
			adc_afe_reg_write(AFE_VAFE_CTRL0, 0x00090b00);
			adc_afe_reg_write(AFE_VAFE_CTRL1, 0x00000110);
			adc_afe_reg_write(AFE_VAFE_CTRL2, 0x0010ef93);
		}
	...
	case FILTER_TVAFE:
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			adc_afe_reg_write(AFE_VAFE_CTRL0, 0x00490710);
			adc_afe_reg_write(AFE_VAFE_CTRL1, 0x0000110e);
			adc_afe_reg_write(AFE_VAFE_CTRL2, 0x1fe09f83);
		} else if (get_cpu_type() == MESON_CPU_MAJOR_ID_TXL ||
			get_cpu_type() == MESON_CPU_MAJOR_ID_TXLX) {
			adc_afe_reg_write(AFE_VAFE_CTRL0, 0x00090b00);
			adc_afe_reg_write(AFE_VAFE_CTRL1, 0x00000110);
...
int adc_set_pll_cntl(bool on, enum adc_sel module_sel, void *p_para)
{
	...
	case ADC_ATV_DEMOD:
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			do {
				if (get_cpu_type() >= MESON_CPU_MAJOR_ID_T5) {
					HIU_WR(HHI_ADC_PLL_CNTL0_TL1,
						0x210504a0);
					HIU_WR(HHI_ADC_PLL_CNTL0_TL1,
						0x310504a0);
					HIU_WR(HHI_ADC_PLL_CNTL1_TL1,
						0x03c00000);
					HIU_WR(HHI_ADC_PLL_CNTL2_TL1,
						0x00000000);
					HIU_WR(HHI_ADC_PLL_CNTL3_TL1,
						0x48681f00);
					HIU_WR(HHI_ADC_PLL_CNTL4_TL1,
						0x88770290);
					HIU_WR(HHI_ADC_PLL_CNTL5_TL1,
						0x39272000);
					HIU_WR(HHI_ADC_PLL_CNTL6_TL1,
						0x56540000);
	...
	case ADC_DTV_DEMOD:
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			do {
				switch (devp->plat_data->chip_id) {
				case ADC_CHIP_T5D:
					adc_set_dtvdemod_pll_by_delsys(devp,
								p_dtv_para);
					break;

				default:
					adc_set_dtvdemod_pll_by_clk(devp,
								p_dtv_para);
					break;
				}
			...
			case SYS_ISDBT:
				if (devp->plat_data->chip_id == ADC_CHIP_T5D)
					dd_tvafe_hiu_reg_write(
						HHI_DEMOD_CLK_CNTL << 2,
						0x507);
				else
					dd_tvafe_hiu_reg_write(
						HHI_DEMOD_CLK_CNTL << 2,
			...
			default:
				dd_tvafe_hiu_reg_write(HHI_DEMOD_CLK_CNTL << 2,
							0x501);
				break;
			}
...
{
	pr_info("AFE_VAFE_CTRL0:0x%x\n", adc_afe_reg_read(AFE_VAFE_CTRL0));
	pr_info("AFE_VAFE_CTRL1:0x%x\n", adc_afe_reg_read(AFE_VAFE_CTRL1));
	pr_info("AFE_VAFE_CTRL2:0x%x\n", adc_afe_reg_read(AFE_VAFE_CTRL2));
	pr_info("HHI_DADC_CNTL:0x%x\n", aml_read_hiubus(HHI_DADC_CNTL));
	pr_info("HHI_DADC_CNTL2:0x%x\n", aml_read_hiubus(HHI_DADC_CNTL2));
	pr_info("HHI_DADC_CNTL3:0x%x\n", aml_read_hiubus(HHI_DADC_CNTL3));
	pr_info("HHI_DADC_CNTL4:0x%x\n", aml_read_hiubus(HHI_DADC_CNTL4));
	pr_info("HHI_S2_DADC_CNTL:0x%x\n", aml_read_hiubus(HHI_S2_DADC_CNTL));
	pr_info("HHI_S2_DADC_CNTL2:0x%x\n",
		aml_read_hiubus(HHI_S2_DADC_CNTL2));
	pr_info("HHI_ADC_PLL_CNTL0_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL0_TL1));
	pr_info("HHI_ADC_PLL_CNTL1_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL1_TL1));
	pr_info("HHI_ADC_PLL_CNTL2_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL2_TL1));
	pr_info("HHI_ADC_PLL_CNTL3_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL3_TL1));
	pr_info("HHI_ADC_PLL_CNTL4_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL4_TL1));
	pr_info("HHI_ADC_PLL_CNTL5_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL5_TL1));
	pr_info("HHI_ADC_PLL_CNTL6_TL1:0x%x\n",
		aml_read_hiubus(HHI_ADC_PLL_CNTL6_TL1));
	pr_info("HHI_DEMOD_CLK_CNTL1:0x%x\n",
		dd_tvafe_hiu_reg_read(HHI_DEMOD_CLK_CNTL1));
	pr_info("HHI_VDAC_CNTL0_T5:0x%x\n",
		aml_read_hiubus(HHI_VDAC_CNTL0_T5));
	pr_info("HHI_VDAC_CNTL1_T5:0x%x\n",
		aml_read_hiubus(HHI_VDAC_CNTL1_T5));
}
