...
static int videotunnel_debug_instance_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "tunnel id=%d, ref=%d\n",
		   instance->id, instance->ref.refcount.counter);
	seq_puts(s, "-----------------------------------------------\n");
					   ...
					   struct file *file)
{
	return single_open(file,
			   videotunnel_debug_instance_show,
			   inode->i_private);
}
...
{
	return single_open(file,
			   videotunnel_debug_session_show,
			   inode->i_private);
}
...
static struct videotunnel_instance
		*videotunnel_instance_create(struct videotunnel_dev *dev)
{
	struct videotunnel_instance *instance;
...
static struct videotunnel_session
		*videotunnel_session_create(struct videotunnel_dev *dev,
					    const char *name)
{
	struct videotunnel_session *session;
	...
	if (kfifo_is_empty(&instance->fifo_to_producer)) {
		ret =
		  wait_event_interruptible_timeout(instance->wait_producer,
						   videotunnel_has_buffer(
						   instance,
						   VT_ROLE_PRODUCER),
						   msecs_to_jiffies(
						   VIDEO_TUNNEL_MAX_WAIT_MS));
		/* timeout */
		if (ret == 0)
			return -EAGAIN;
	}
				      ...
				      struct videotunnel_session *session)
{
	...
	if (kfifo_is_empty(&instance->fifo_to_consumer)) {
		ret =
		  wait_event_interruptible_timeout(instance->wait_consumer,
						   videotunnel_has_buffer(
						   instance,
						   VT_ROLE_CONSUMER),
						   msecs_to_jiffies(
...
static int __init meson_videotunnel_init(void)
{
...
static void __exit meson_videotunnel_exit(void)
{
...
MODULE_DESCRIPTION("AMLOGIC videotunnel driver");
MODULE_LICENSE("GPL");
