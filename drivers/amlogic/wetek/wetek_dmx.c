...
int dsc_set_pid(struct wetek_dsc *ch, int pid)
{
	struct wetek_dsc *dsc = ch;
...
int dsc_set_key(struct wetek_dsc *ch, int type, u8 *key)
{
	struct wetek_dsc *dsc = ch;
	int is_dsc2 = (dsc->id == 1) ? 1 : 0;
	u16 k0, k1, k2, k3;
...
int dsc_enable(struct wetek_dsc *dsc, int enable)
{
	if (dsc->id == 0) {
		WRITE_MPEG_REG(STB_TOP_CONFIG,
...
int dsc_release(struct wetek_dsc *dsc)
{
	return 0;
}
...
static int dmx_set_misc(struct wetek_dmx *dmx, int hi_bsf, int en_dsc)
{
	if (hi_bsf >= 0) {
		DMX_WRITE_REG(dmx->id, TS_HIU_CTL,
					hi_bsf ?
...
static int dmx_set_misc_id(struct wetek_dvb *dvb, int id, int hi_bsf, int en_dsc)
{
...
void dmx_reset_hw_ex(struct wetek_dvb *dvb, int reset_irq)
{
	...
	{
		struct wetek_dsc *dsc = &dvb->dsc[id];
...
int wetek_dsc_hw_set_source(struct wetek_dvb *dvb, dmx_source_t src)
{
	int ret = 0;
	unsigned long flags;
