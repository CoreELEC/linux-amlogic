/*
 * drivers/amlogic/amlnf/phy/new_nand.c
 *
...
#if 0
static int aml_nand_get_20nm_OTP_value(struct hw_controller *controller,
	unsigned char *buf,
	unsigned char chipnr)
{
	...
	aml_nand_dbg("20 nm flash total_reg_cnt:%d, reg_cnt_otp:%d, chip[%d]",
		total_reg_cnt,
		reg_cnt_otp,
		chipnr);
	...
	if (check_flag) {
		aml_nand_msg("20nm flash def vaule abnormal not safe !chip[%d]",
			chipnr);
		BUG();
	} else {
		tmp_buf = buf;
		aml_nand_dbg("20 nm flashdefault vaule OK at %dst copy", i);
		memcpy(&retry_info->reg_def_val[chipnr][0],
			tmp_buf,
			retry_info->reg_cnt_lp);
		aml_nand_dbg("20 nm flash default vaule");
		for (j = 0; j < retry_info->reg_cnt_lp; j++)
			aml_nand_dbg("REG(0x%x): value:0x%2x, for chip[%d]",
				retry_info->reg_addr_lp[j],
				retry_info->reg_def_val[chipnr][j],
				chipnr);
	...
	for (j = 0; j < retry_info->retry_cnt_lp; j++) {
		for (k = 0; k < retry_info->reg_cnt_lp; k++) {
			retry_info->reg_offs_val_lp[chipnr][j][k] =
				(char)tmp_buf[0];
			tmp_buf++;
			aml_nand_dbg("Retry[%d] REG(0x%x):value:0x%2x,chip[%d]",
				j,
				retry_info->reg_addr_lp[k],
				retry_info->reg_offs_val_lp[chipnr][j][k],
				chipnr);
		}
			aml_nand_dbg("retry_info->retry_cnt_lp:%d",
...
for (i = 0; i < HYNIX_OTP_COPY; i++) {
	for (j = 0; j < 8; j++) {
		for (k = 0; k < 4; k++) {
			...
			if ((unsigned char)(buf[m]^buf[m+32]) == 0xFF) {
				aml_nand_dbg("%dg %dretry %dval ok,buf 0x%02x",
					i, j, k,
					buf[m]);
			if (j == 0)
				retry_info->reg_def_val[chipnr][k] = buf[m];
			else
				retry_info->reg_offs_val_lp[chipnr][j-1][k] =
					buf[m];
	...
	for (i = 0; i < 32; i++)
		if (retry_value_sta[i] == 0) {
			aml_nand_msg("chip[%d] %d vaule abnormal not safe!",
				chipnr,
				i);
			return 1;
		}
...
int aml_nand_sandisk_A19_set_LMFLGFIX_NEXT(struct hw_controller *controller,
	unsigned char chipnr)
{
	int ret = 0;
	...
	/*Test Mode Enter*/
	controller->cmd_ctrl(controller,
		NAND_CMD_SANDISK_TEST_MODE1,
		NAND_CTRL_CLE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->cmd_ctrl(controller,
		NAND_CMD_SANDISK_TEST_MODE2,
		NAND_CTRL_CLE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->cmd_ctrl(controller,
		NAND_CMD_SANDISK_TEST_MODE_ACCESS,
		NAND_CTRL_CLE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->cmd_ctrl(controller, 0x00, NAND_CTRL_ALE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->writebyte(controller, 0x01);
	NFC_SEND_CMD_IDLE(controller, 10);
	...
	controller->cmd_ctrl(controller,
		...
		NAND_CTRL_CLE); /* 0X55 */
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->cmd_ctrl(controller, 0x23, NAND_CTRL_ALE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->writebyte(controller, 0xC0);
	NFC_SEND_CMD_IDLE(controller, 10);
	ret = controller->quene_rb(controller, chipnr);
	if (ret) {
		aml_nand_msg("quene rb failed chipnr:%d", chipnr);
...
int aml_nand_sandisk_A19_reset_LMFLGFIX_NEXT(struct hw_controller *controller,
	unsigned char chipnr)
{
	int ret = 0;
	...
	controller->cmd_ctrl(controller, 0x23, NAND_CTRL_ALE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->writebyte(controller, 0x40);
	NFC_SEND_CMD_IDLE(controller, 10);

	/*Test Mode Exit*/
	controller->cmd_ctrl(controller,
		NAND_CMD_SANDISK_TEST_MODE_ACCESS, NAND_CTRL_CLE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->cmd_ctrl(controller, 0x00, NAND_CTRL_ALE);
	NFC_SEND_CMD_IDLE(controller, 2);
	controller->writebyte(controller, 0x00);
	NFC_SEND_CMD_IDLE(controller, 10);
	ret = controller->quene_rb(controller, chipnr);
	if (ret) {
		aml_nand_msg("quene rb failed chipnr:%d", chipnr);
	...
	unsigned char chipnr)
{
	...
	case 3:
		...
		break;
	default:
		aml_nand_msg("never enter,retry stage:%d\n",
...
int amlnand_set_readretry_slc_para(struct amlnand_chip *aml_chip)
{
	...
	#if 0
		printk("++ Read Retries %d :", retry_info->retry_cnt_lp);
		for (i = 0; i < retry_info->retry_cnt_lp; i++)
