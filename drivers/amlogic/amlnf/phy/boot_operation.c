/*
 * drivers/amlogic/amlnf/phy/boot_operation.c
 *
...
static int read_uboot(struct amlnand_phydev *phydev)
{
	...
	unsigned int configure_data, pages_per_blk, configure_data_w;
	unsigned int pages_per_blk_w, page_size, tmp_size;
	...
	int ret = 0;
	unsigned int tmp_value, en_slc = 0, tmp_index;
	...
	while (1) {
		...
		/* controller->chip_num; */
		ops_para->page_addr = ((unsigned int)addr / flash->pagesize);
		...
		if ((!strncmp((char *)phydev->name,
			...
			memcpy((unsigned char *)(&configure_data_w),
				ops_para->data_buf,
				sizeof(int));
			memcpy((unsigned char *)(&pages_per_blk_w),
				ops_para->data_buf+4,
				sizeof(int));
...
static int write_uboot(struct amlnand_phydev *phydev)
{
	...
	unsigned char *oob_buf = NULL, *page0_buf = NULL;
	unsigned char tmp_bch_mode, tmp_user_mode, tmp_ecc_limit, tmp_ecc_max;
	unsigned char tmp_rand, tmp_index;
	unsigned int configure_data, pages_per_blk;
	unsigned int oobsize, page_size, tmp_size, priv_lsb, ops_tem;
	unsigned short tmp_ecc_unit, tmp_ecc_bytes, tmp_ecc_steps;
	...
	unsigned int tmp_value, fill_len = 0;
			...
			if (writelen >= len) {
				...
				if ((mt_L95B_nand_check(aml_chip) == 0)
					...
					for (j = 0; j < 11; j++) {
						...
						if (ret < 0) {
							write_boot_status[i] = 1;
							aml_nand_msg("write fail page_addr:%d", ops_para->page_addr);
							break;
						}
						addr += flash->pagesize;
					}
