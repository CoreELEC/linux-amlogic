/*
 * drivers/amlogic/amlnf/phy/chip_operation.c
 *
...
static int ecc_read_retry_handle(struct amlnand_chip *aml_chip,
	...
	unsigned char user_byte_num,
	unsigned int page_size,
	unsigned char slc_mode,
	unsigned char up_page)
{
	struct hw_controller *controller = &(aml_chip->controller);
	...
	unsigned int plane1_page_addr)
{
	...
	unsigned int page0_addr, page1_addr;
	unsigned char plane0_retry_flag = 0, all_ff_flag = 0;
	unsigned char bch_mode, user_byte_num, slc_mode, up_page = 0;
	int ret = 0;
	int new_oob = 0;
...
static int read_page(struct amlnand_chip *aml_chip)
{
	struct hw_controller *controller = &(aml_chip->controller);
	struct chip_ops_para *ops_para = &(aml_chip->ops_para);
	unsigned int plane0_page_addr, plane1_page_addr, page_addr;
	...
	int value)
{
	/* struct hw_controller *controller = &aml_chip->controller; */
	/* struct nand_flash *flash = &aml_chip->flash; */
	unsigned int blk_addr = addr;
	unsigned short *tmp_status = NULL;
...
static int erase_block(struct amlnand_chip *aml_chip)
{
	...
	struct en_slc_info *slc_info = &(controller->slc_info);
	unsigned int plane_page_addr, plane_blk_addr, pages_per_blk_shift;
	unsigned int plane0_page_addr, plane1_page_addr, chipnr;
...
static int test_block_chip_op(struct amlnand_chip *aml_chip)
{
	...
	unsigned int pages_per_blk, pages_read, blk_addr = 0;
	unsigned char  oob_buf[8];
	int  ret = 0, t = 0;
