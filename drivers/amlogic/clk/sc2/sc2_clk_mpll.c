/*
 * drivers/amlogic/clk/sc2/sc2_clk_mpll.c
 *
...
#define SC2_MPLL_CNTL0 0x00000543
#define SC2_MPLL_CNTL2 0x40000033
			 ...
			 unsigned long parent_rate)
{
	...
	writel(SC2_MPLL_CNTL0, mpll->base + mpll->mpll_cntl0_reg);

	p = &mpll->sdm;
	writel(SC2_MPLL_CNTL2, mpll->base + p->reg_off +
	       (unsigned long)(1 * 4));
	reg = readl(mpll->base + p->reg_off);
...
const struct clk_ops meson_sc2_mpll_ops = {
...
const struct clk_ops meson_sc2_mpll_ro_ops = {
	.recalc_rate = mpll_recalc_rate,
};
