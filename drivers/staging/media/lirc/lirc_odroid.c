...
static long lirc_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case LIRC_SET_SEND_DUTY_CYCLE:
		...
		if (value <= 0 || value > 100)
			...
			result = init_timing_params(value, freq);
		break;
	...
	case LIRC_SET_SEND_CARRIER:
		...
		if (value > 500000 || value < 20000)
			...
			result = init_timing_params(duty_cycle, value);
		break;

	default:
		result = lirc_dev_fop_ioctl(filep, cmd, arg);
		break;
	}
...
MODULE_DESCRIPTION("GPIO based IR Transmitter driver for Odroid C/C2 and N2");
MODULE_AUTHOR("Joy Cho <joy.cho@hardkernel.com>");
MODULE_LICENSE("GPL");

module_param(gpio_out_pin, int, 0644);
MODULE_PARM_DESC(gpio_out_pin, "GPIO output/transmitter pin number");

module_param(softcarrier, bool, 0644);
