...
static int ir_irmp_decode(struct rc_dev *dev, struct ir_raw_event ev)
{
	u32 scancode;
	IRMP_DATA irmp_data;
	unsigned int pulse = ev.duration;
	u8 protocol = 0xFF;
	...
	dev_dbg(&dev->dev, "IRMP decode %uus (%uus): %s\n",
		   pulse, ev.duration, TO_STR(ev.pulse));

	// ready with decode
	if (irmp_ISR(pulse ? ((pulse * 15) / 1000) : 0)) {
		if (irmp_get_data(&irmp_data)) {
			dev_info(&dev->dev, "IRMP decoded by protocol %s, command %04x, address %04x",
			...
			rc_keydown(dev, RC_PROTO_IRMP, scancode, 0);
