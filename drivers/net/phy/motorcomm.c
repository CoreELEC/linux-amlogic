...
int yt8511_config_dis_txdelay(struct mii_bus *bus, int phy_id)
{
    ...
    */
    ...
    pr_info("yt8511_config_dis_txdelay..phy txdelay, val=%#08x\n",val);
...
int yt8511_config_out_125m(struct mii_bus *bus, int phy_id)
{
	...
	if(val != 0xd0)
	{
		pr_info("yt8511_config_out_125m error\n");
		return -1;
	}
	...
	*/
	val |= (3 << 1);
	ret = ytphy_mii_wr_ext(bus, phy_id, 0xc, val);
	pr_info("yt8511_config_out_125m, phy clk out, val=%#08x\n",val);
	...
	pr_info("yt8511_config_out_125m, phy to be master, val=%#08x\n",val);
#endif
...
static int yt8521_config_init(struct phy_device *phydev)
{
	...
	pr_info("yt8521_config_init, 8521 init call out.\n");
	return ret;
}
...
static int yt8521_adjust_status(struct phy_device *phydev, int val, int is_utp)
{
	...
	phydev->speed = speed;
	phydev->duplex = duplex;
	//pr_info("yt8521_adjust_status call out,regval=0x%04x,mode=%s,speed=%dm...\n", val,is_utp?"utp":"fiber", phydev->speed);
...
int yt8521_aneg_done (struct phy_device *phydev)
{
	...
	//pr_info("yt8521_aneg_done callin,speed=%dm,linkmoded=%d\n", phydev->speed,link_mode_8521);
...
static int yt8521_read_status(struct phy_device *phydev)
{
	...
	//pr_info("yt8521_read_status, fiber status=%04x,macbase=0x%08lx\n", val,(unsigned long)phydev->attached_dev);
	...
	{
		link = 0;
		pr_info("yt8521_read_status, fiber link down detect,latch=%04x,curr=%04x\n", yt8521_fiber_latch_val,yt8521_fiber_curr_val);
	}
	...
	//pr_info("yzhang..8521 read status call out,link=%d,linkmode=%d\n", phydev->link, link_mode_8521 );
	return 0;
}
...
static int yt8618_config_init(struct phy_device *phydev)
{
...
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(4,0,0) )
	{
		...
		pr_info("yzhang..8618 init, phy address mismatch, base=%d, cur=%d\n", yt_mport_base_phy_addr, phydev->addr);
	}
#else
	{
	...
	pr_info("yt8618_config_init call out.\n");
	return ret;
}
...
static int yt8614_config_init(struct phy_device *phydev)
{
...
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(4,0,0) )
	{
		...
		pr_info("yzhang..8618 init, phy address mismatch, base=%u, cur=%d\n", yt_mport_base_phy_addr_8614, phydev->addr);
	}
#else
	{
...
static int yt8614_read_status(struct phy_device *phydev)
{
	...
	//pr_info("yzhang..8614 read fiber status=%04x,macbase=0x%08lx\n", val,(unsigned long)phydev->attached_dev);
	...
	{
		link = 0;
		pr_info("yt8614_read_status, fiber link down detect,latch=%04x,curr=%04x\n", yt8614_fiber_latch_val,yt8614_fiber_curr_val);
	}
...
#endif
	//pr_info("yt8614_read_status call out,link=%d,linkmode=%d\n", phydev->link, link_mode_8614[port] );
